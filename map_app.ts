...
const EXAMPLE_PROMPTS = [
  'Show me directions from Tokyo Tower to Shibuya Crossing and check the weather.',
  'Can you show me a beautiful beach?',
  'Show me San Francisco',
  'Give me directions from the Eiffel Tower to the Louvre Museum, are there any traffic delays?',
  ...
  "What's the way from Buckingham Palace to the Tower of London? Will I hit any rain?",
  ...
  'How do I get from Times Square, New York to Central Park? Check for rain.',
  'Show me the route from the Golden Gate Bridge to Alcatraz Island, and give me a traffic report.',
];
...
You can find this constant near the top of the map_app.ts file.`;
      ...
      // FIX: Removed redundant call to requestUpdate(). Updating the @state property `mapError` above is sufficient to trigger a re-render.
      return;
    }
    ...
    // FIX: Removed redundant call to requestUpdate(). Updating @state properties within the try/catch block is sufficient to trigger a re-render.
  }
      ...
      async (response: any, status: string) => {
        if (
          ...
          if (route.overview_path && this.Polyline3DElement) {
            ...
            this.routePolyline.path = pathCoordinates;
